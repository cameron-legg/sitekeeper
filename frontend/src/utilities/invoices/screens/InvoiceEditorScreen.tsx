...
import { useSavedItems, usePopulateSavedItem, useSaveEntryToLibrary, usePopulateSavedEntry, useAllSavedEntries } from "../../saved_items/hooks/useSavedItems";
import { useJob } from "../../../core/api/hooks/useJobs";
import { useDocumentFieldSettings, type FieldVisibility } from "../../../core/api/hooks/useDocumentSettings";
...
export default function InvoiceEditorScreen({ route, navigation }: Props) {
  ...
  const { data: fieldSettings } = useDocumentFieldSettings("invoice");
  ...
  function handleCreate() {
    ...
    createInvoice.mutate({ jobId, title: t, tax_rate: taxRate.trim() || undefined }, {
      onSuccess: (inv) => {
        ...
        setShowDocumentNumber(inv.show_document_number);
        setShowDocumentDate(inv.show_document_date);
        setShowBillTo(inv.show_bill_to);
        setShowCompanyName(inv.show_company_name);
        setShowUserName(inv.show_user_name);
        setShowUserPhone(inv.show_user_phone);
        setShowUserEmail(inv.show_user_email);
        setShowPaymentMethod(inv.show_payment_method);
