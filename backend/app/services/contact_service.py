...
from ..repositories.contact_repo import IContactRepository, SQLAlchemyContactRepository
...
class ContactService:
    ...
    def get_contacts_for_job(self, job_id: str, user_id: str) -> list[Contact]:
        ...
        if site is None:
            raise NotFoundError(f"Job {job_id} not found.")
        return self._contact_repo.get_for_job(job_id)
    ...
    def get_effective_primary_contact(
        ...
        Returns:
            dict with keys 'contact' (Contact model) and 'source' ('direct' or
            'inherited'), or None if no primary contact is set anywhere.
        """
        job = self._job_repo.get_by_id(job_id)
        if job is None:
            raise NotFoundError(f"Job {job_id} not found.")
